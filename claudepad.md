...
### 2026-06-19 — Fix: vuln-negotiation seller-floor inversion (unported sibling of the session.py bug) + attestation nonce fail-open
- **Approach.** Fanned out 5 read-only auditor sub-agents over subsystems *not* recently scrutinized (negotiation math, poker evaluator/views, crypto/messaging, policy/recall, attestation/COSE), then verified each finding against source before fixing. Fixed the two cleanest, highest-confidence, reachable correctness bugs; deferred the murkier/architectural ones (see below). Adversarial 6th sub-agent re-reviewed the diff: **SOUND**.
- **Bug 1 — Vuln negotiation: below-floor lowball silently accepted as final price (HIGH, headline "Sealed 0day verification" flow).** Exact same bug class as the `session.py` floor inversion fixed on 2026-06-17, **but never ported to `vuln_session.py`**. `VulnSellerAgent.evaluate_offer` hard-rejects sub-floor offers by short-circuiting WITHOUT the LLM → returns a counter-at-floor msg with `raw_response=None` + `price_proposal` set. The vuln round loop's inline `if seller_response.raw_response: ... else: action="accept"` read that as **accept**, so a lowball `< alpha_0·omega_hat` became `candidate_price` → final deal price, violating the seller's documented floor. **Fix:** ported `NegotiationSession._seller_action` verbatim into `VulnNegotiationSession` (`raw_response=None` + `price_proposal is not None` ⇒ `"counter"`), replacing the inline logic. Below-floor lowballs now fall through to the bilateral Nash backstop `(v_b + alpha_0·omega_hat)/2` (always ≥ floor since the round loop only runs after viability `v_b ≥ alpha_0·omega_hat`) or NO_DEAL when budget < backstop. Was **untested** (the floor regressions only covered `session.py`).
- **Bug 2 — Attestation: nonce type-confusion fail-open (defense-in-depth, trust root).** `_verify_nitro_cbor`'s nonce check only handled `doc_nonce` of type `bytes` or `str`; **any other CBOR type (int/list/bool) skipped validation entirely** → an attacker-shaped payload defeats freshness/replay binding. **Fix:** compute a `nonce_matches` bool with `else: nonce_matches = False` → fail closed on unexpected type. (`SimulatedAttestationVerifier.verify` already fails closed via `!=`; left as-is.)
- **Tests (+6; 861→867 unit).** `test_vuln_session_floor.py` (new, 5: `_seller_action` truth table mirroring the session.py test + two end-to-end full-`run()` tests driving real `evaluate_offer` floor logic with only the LLM-driven methods stubbed — lowball never settles sub-floor; budget<backstop ⇒ NO_DEAL); `test_nonce_type_confusion_fails_closed` in `test_cose_verification.py` (int nonce ⇒ "Nonce mismatch"). All fail-before/pass-after verified by reintroducing each bug. Integration 59 pass / 26 skip; **ruff: no new errors** (2 pre-existing E501 in touched files; new test file clean); **mypy flat 542**.
- **Deferred (real but architectural / risky for a targeted pass, flagged by auditors):** (a) `verify_attestation` selects regime by *sniffing* input bytes — a forged JSON `{"type":"simulated_attestation"}` would take the no-crypto path; fixing needs a mode-gated Nitro-only entry point and risks breaking simulated mode (the whole suite's default). (b) `expected_pcrs` defaulted `None` in all prod callers ⇒ PCR identity never constrains server-side (same family as the long-deferred "PCR pinning not passed in Nitro"). (c) Policy enforcement: `secret_proxy`/`transcript_processor` compute `policy_report.all_passed` but hardcode `success=True` and return output regardless — but naive gating over-blocks because the LLM generator can add constraints for fields not passed to `enforce_all` (false-positive risk); needs a field-scoped fix. (d) Messaging OTPK reuse: no unique constraint on `(owner_pubkey, otpk_index)` + client index math can re-upload unconsumed indices; needs an alembic migration + frontend change. Poker evaluator/views audited clean (200k-hand differential fuzz, no view leak).

### 2026-06-18 — Fix: two deferred vuln-verify bugs (false-positive DoS/crash on PoC timeout; verdict ignored claimed capability)
- **Picked up two of the three remaining "deferred" bugs from the 2026-06-17 session** (the third — PCR pinning not passed to attestation in Nitro — stays deferred, untestable in simulated mode). Both are in the headline **Sealed 0day verification** subsystem and both are reachable, security-relevant correctness bugs with no prior HTTP/enclave-path test coverage.
- **Bug A — A PoC that merely hangs was credited with a capability (false "verified").** Two conflations: (1) on timeout the enclave SIGKILLs the *PoC* process → `PoCResult.signal=9` → crash oracle saw `signal is not None` → false CRASH; (2) `oracles.check_result` had `dos = poc_result.timed_out`, so any slow/hung PoC (`sleep 9999`) → false DOS. A seller could "verify" a worthless exploit. **Fix:** (a) `poc_executor.execute_poc` no longer surfaces `signal` when *it* killed the PoC on timeout (only genuine non-timeout signal deaths set `signal`; `exit_code` still `128+sig` for diagnostics); (b) real DoS oracle — new `PoCExecutor.check_services_healthy()` re-runs each service health check after the PoC; `oracles.check_result` gains `target_unresponsive` (replaces the `timed_out` derivation); (c) `protocol._run_with_oracles` credits DoS only if the target was healthy **before** the PoC and unhealthy **after** (so a service that never started isn't mistaken for PoC-induced DoS).
- **Bug B — Orchestrator verdict ignored the claimed capability level.** `vuln_verify_orchestrator` computed `unpatched_matches = verified_level is not None` in both `_run_simulated` and `_run_nitro`, so claiming LPE but only achieving CRASH still "passed" (this bool flows to `nitro_verifier` passed/failed status, the API result, and the on-chain commitment). The protocol layer already preserved `claimed` vs `verified_level` correctly — only the orchestrator's bool collapse was wrong. **Fix:** new `protocol.capability_meets_claim(verified, claimed)` (verified rank ≥ claimed rank, via the existing `_CAP_RANKS`; `None` never matches), wired into both paths. Nitro path uses its *own* `config.target_spec.claimed_capability.level` (doesn't trust the enclave-returned claim) + `_level_from_value` (string→enum, fail-closed on unknown).
- **Tests (+29; 832→861 unit):** `test_vuln_verify_verdict.py` (new, 20 tests: full `capability_meets_claim` truth table + `_level_from_value` + end-to-end `_run_simulated` verdict via stub provider & patched `protocol.run`); oracle false-positive regressions (hung PoC ⇒ no DoS/crash; DoS keys on target health not PoC timeout); executor `test_timeout_does_not_report_crash_signal` + `check_services_healthy` pass/fail/empty; protocol DoS-wiring (healthy-then-down ⇒ DoS; timeout+healthy ⇒ none; never-healthy ⇒ none). Integration 59 pass; **ruff flat 404**; **mypy 545→542**.
- **Adversarial sub-agent review: no real bugs.** Confirmed timeout-signal change masks no genuine crash (only the enclave's own SIGKILL), `check_outcome` has no prod caller, `.claimed` always equals the spec level on both paths, no missed consumers, no circular import. One latent non-bug flagged (pre-existing): `_CAP_RANKS` orders LPE(6)>CALLBACK(5) while the enum docstring implies CALLBACK highest — the disagreeing pairs are **unreachable** (oracle planting starts the callback listener only for CALLBACK claims, plants the LPE canary only for LPE claims), so left as-is; resolving it is a separate semantic call affecting `_cap_rank`'s existing best-result selection.

### 2026-06-17 — Fix: poker sub-minimum all-in illegally reopened betting (deferred bug from prior session)
- **Bug (HIGH, headline "provably fair dealer" feature).** Texas Hold'em rule: an all-in *raise of less than a full raise* must NOT reopen betting to players who already acted — they may only call or fold. The engine reset `has_acted=False` for everyone on any wager-increasing action and `get_valid_actions` consulted only `to_call`/stack, so after a short all-in an already-acted seat was offered (and `process_action` would **accept**) an illegal re-raise/shove. This was the deferred poker item from the previous session ("needs a real betting-reopened flag — `get_valid_actions` ignores `has_acted`"). Reproduced: P0 raises to 300 (min-raise 200), P1 short-all-ins to 350 (raise of 50), P2 calls → P0 was offered `raise`+`all_in` (should be `fold`/`call` only). P2, who hadn't acted, correctly keeps full rights.
- **Root cause vs. enforcement.** Critically, `get_valid_actions` is only used by tests/frontend; the authoritative boundary is `process_action`→`_do_raise`/`_do_all_in`, which had **no** reopen check. So menu-hiding alone would not have fixed the enclave — an adversarial/buggy client could still submit the raise.
- **Fix.** New absolute-bet-level state: `HandState.reopen_bet` (level set by the last *full* bet/raise) and `PlayerSeat.acted_at_bet` (level when the seat last *voluntarily* acted; -1 = not yet — forced blinds don't count). Helper `_betting_reopened(hand, seat) = acted_at_bet < reopen_bet`. A full raise (`raise_size >= min_raise`, computed before mutating `min_raise`) advances `reopen_bet`; a short all-in leaves it. Gated in `get_valid_actions` **and enforced** in `_do_raise`/`_do_all_in` (raise `PokerEngineError("Betting not reopened …")`). All-in *calls/under-calls* (`new_total <= current_bet`) stay always-legal; only all-in *raises* are gated. Resets: `acted_at_bet=-1` + `reopen_bet=big_blind` preflop (preserves the BB option since blinds don't set `acted_at_bet`), `acted_at_bet=-1` + `reopen_bet=0` each new street.
- **Tests/verification.** +11 tests (`tests/unit/test_poker_reopen.py`): scenario (short all-in freezes acted player / unacted player keeps rights / full re-raise reopens / BB option / postflop), **enclave-enforcement** tests that bypass the menu and assert `process_action` rejects illegal raise+shove, and two fuzz tests (every offered action is processable; zero-sum under gating). Fail-before/pass-after confirmed by reintroducing the bug (2 targeted tests flipped red). Adversarial probe: 4000 hands via `process_action`, 221 frozen-seat situations, **221/221 illegal raise+shove rejected**, zero-sum held. Independent 4th sub-agent review (~100k fuzzed hands): "**No correctness issues found.**" Unit 821→**832 pass**; integration 59 pass; ruff flat 404; mypy flat 545. Self-contained to the poker engine (no view/SSE/serialization consumers of the new state).

### 2026-06-17 — Fix: three correctness bugs across negotiation, poker, vuln-verify (each with regression tests)
- **Approach**: Fanned out 3 bug-hunting sub-agents over the highest-risk subsystems (negotiation core, poker engine, attestation/vuln-verify), then independently verified each finding against source + empirical probes before fixing. Fixed only the objective, reachable, test-coverable bugs; deferred the murkier ones (see below).
- **Bug 1 — Negotiation: seller's hard floor silently inverted (HIGH, default config).** `SellerAgent.evaluate_offer` enforces the floor by *early-returning* a counter-at-floor message with `raw_response=None` for below-floor offers (no LLM call). `session.py` read a missing `raw_response` as `action="accept"`, so a buyer lowball **below** the seller's floor (alpha_0·omega_hat) became the final deal price. Affects the default `max_rounds=5`. Fix: extracted `NegotiationSession._seller_action()` — a below-floor counter (raw_response=None + price_proposal) now reads as `"counter"`, never `"accept"`. Below-floor lowballs now fall through to the Nash backstop (≥ floor) or NO_DEAL when budget < floor.
- **Bug 2 — Vuln-verify: buyer overlay decryption always crashed in Nitro mode (HIGH).** `overlay_handler.decrypt_overlay` passed the whole `EnclaveKeypair` dataclass to `ecies_decrypt` (which calls `private_key.exchange(...)`) → `AttributeError` on every real-mode overlay decrypt. Every other call site passes `.private_key`. Fix: one-liner `self._keypair.private_key`. Survived because `decrypt_overlay` had only a no-keypair test — added an ECIES round-trip test.
- **Bug 3 — Poker: chips destroyed at showdown (HIGH, zero-sum violation).** When a top side pot above the highest active all-in is funded only by folded over-bettors, `eligible_players` is empty and `_showdown` dropped it (`if not eligible_scores: continue`) — destroying chips. Empirically reachable: random-play fuzz hit 9/4000 hands. Fix: new `_return_uncalled_bets()` returns the uncalled excess (total above max active total) to its contributors before pot collection, in both `_showdown` and `_end_hand_last_standing`; idempotent (caps total_bet_this_hand). Emits a benign `uncalled_bet_returned` event (only seat+amount, no private data; forwarded generically by the SSE layer). Post-fix fuzz: 0/8000 violations.
- **Deferred (real but not safe/clean for this pass)**: oracles.py timeout→SIGKILL counted as crash→false "verified" (needs DoS-detection redesign); orchestrator verdict ignores claimed capability level; PCR pinning never passed to attestation in Nitro (untestable in simulated mode); sub-min all-in "reopens" betting (the agent's proposed `has_acted` guard wouldn't actually change `get_valid_actions`, which ignores `has_acted` — needs a real betting-reopened flag).
- **Tests**: +9 regression tests (test_session_floor.py ×5, test_poker_zero_sum.py ×3, overlay round-trip ×1), all fail-before/pass-after verified by reintroducing each bug. Fixed test_multi_round budget-clamp test that had been *asserting* Bug 1 (expected accept at below-floor 0.01 → now uses feasible budget 0.3). Unit 812→**821 pass**; integration 59 pass; mypy flat at 545; ruff 405→404. Adversarially re-reviewed by a 4th sub-agent: "DIFF LOOKS CORRECT".
